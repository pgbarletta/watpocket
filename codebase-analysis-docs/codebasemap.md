...
- For SKBUILD wheel builds, top-level CMake package/CLI install rules are skipped (`CMakeLists.txt` early return on `SKBUILD`) so Python wheels do not accidentally include CMake package artifacts; Python install rules now place `watpocket_ext` and `watpocket_lib` together in package root with module `INSTALL_RPATH=$ORIGIN`, while native CMake `pythoninstall` keeps `watpocket_lib` in `platlib/lib` with `INSTALL_RPATH=$ORIGIN/lib` (source: `CMakeLists.txt`, `src/python/CMakeLists.txt`).
