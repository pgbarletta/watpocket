...
- Build system is CMake; top-level adds `configured_files`, `src`, and optionally `test`, while project version/git SHA are now simple inputs (`WATPOCKET_VERSION`, `GIT_SHA`) instead of being discovered by shelling out to `git` during configure (source: `CMakeLists.txt`).
- Runtime entrypoint for project deliverable is `src/watpocket/main.cpp` (`main`) (source: `src/watpocket/main.cpp:main`).
- `src/CMakeLists.txt` always builds `watpocket_lib`, conditionally builds CLI target `watpocket` when `WATPOCKET_BUILD_CLI=ON`, and conditionally adds `src/python` when `WATPOCKET_ENABLE_PYTHON_BINDINGS=ON` (source: `src/CMakeLists.txt`).
...
- Hardening on Linux can link `libssp` and add its directory to the build RPATH when stack protector support is available so shared objects resolve `__stack_chk_guard` at import time (source: `cmake/Hardening.cmake`).
- CPM remains enabled only for targets that actually need remote packages: Catch2 is fetched only when `BUILD_TESTING=ON`, and CLI11 only when `WATPOCKET_BUILD_CLI=ON`; wheel/editable Python builds avoid including `cmake/CPM.cmake` at all because they disable both surfaces structurally in `Dependencies.cmake` (source: `Dependencies.cmake`, `cmake/CPM.cmake`).
- CPM bootstrap prefers a cached `CPM.cmake` path from `CPM_SOURCE_CACHE` and otherwise downloads the file; there is no longer a wheel-only no-op fallback in `cmake/CPM.cmake` because wheel mode now bypasses CPM by structure instead of by stub logic (source: `Dependencies.cmake`, `cmake/CPM.cmake`).
...
- CMake option `WATPOCKET_BUILD_CLI` gates the CLI executable target; scikit-build (`SKBUILD`) forces it off so Python wheel builds avoid compiling/linking the CLI and its `CLI11` dependency path (source: `CMakeLists.txt`, `src/CMakeLists.txt`).
<<<<<<< HEAD
- `BUILD_TESTING` is forced off under `SKBUILD` to prevent packaging-time configure/generate failures from CLI-focused CTest entries that reference target `watpocket` (source: `CMakeLists.txt`, `test/CMakeLists.txt`).
- Python packaging surface is defined in `pyproject.toml` using `scikit-build-core` with build targets `watpocket_ext` and `watpocket_stub` (output module name `watpocket` plus generated `watpocket.pyi`, while keeping install rules satisfied); Python package version is dynamic and comes from an exact git tag (`vX.Y.Z`/`X.Y.Z`) via `scripts/pypi_version_provider.py`; editable/wheel builds disable CLI/tests/sanitizers/analyzers through a short set of explicit CMake defines, and the extension installs with `RPATH=$ORIGIN` in `SKBUILD` mode so bundled `libwatpocket.so` resolves from the wheel root at import time (source: `pyproject.toml`, `scripts/pypi_version_provider.py`, `src/python/CMakeLists.txt`, `ProjectOptions.cmake`).
=======
- `BUILD_TESTING` is forced off under `SKBUILD` to prevent packaging-time configure/generate failures from CLI-focused CTest entries that reference target `watpocket`; `include(CTest)` is called before `myproject_setup_dependencies()` so `BUILD_TESTING` is available when Catch2 fetch is gated (source: `CMakeLists.txt`, `test/CMakeLists.txt`).
- Python packaging surface is defined in `pyproject.toml` using `scikit-build-core` with build targets `watpocket_ext` and `watpocket_stub` (output module name `watpocket` plus generated `watpocket.pyi`, while keeping install rules satisfied); Python package version is dynamic and comes from an exact git tag (`vX.Y.Z`/`X.Y.Z`) via `scripts/pypi_version_provider.py`; the extension installs with `RPATH=$ORIGIN/lib` so bundled `libwatpocket.so` resolves at import time, and editable/wheel builds default to `Release` with `clang-tidy`, `cppcheck`, and ASan/UBSan disabled via CMake defines for faster package builds (source: `pyproject.toml`, `scripts/pypi_version_provider.py`, `src/python/CMakeLists.txt`, `ProjectOptions.cmake`).
>>>>>>> 194588d1 (fix)
- `pyproject.toml` sets `tool.scikit-build.experimental = true` to allow the custom dynamic metadata version provider plugin used for tag-driven versions.
- Python dev extra (`.[dev]`) provides release tooling dependencies (`build`, `twine`) used by wheel build/upload workflows in `scripts/pypi_release_wheels.sh`; on Linux it also installs `auditwheel` for manylinux tag repair before upload (source: `pyproject.toml`, `scripts/pypi_release_wheels.sh`).
- Release helper `scripts/pypi_release_wheels.sh` is now a single local wheel-release entrypoint: it enforces clean-tree exact-tag releases, resolves the interpreter (`.venv/bin/python`, then `python3`, then `python`), injects `WATPOCKET_VERSION` + `GIT_SHA` into CMake via `CMAKE_ARGS`, builds with `python -m build --wheel --no-isolation`, repairs Linux wheels with `auditwheel`, and uploads wheel files only with `twine` (no source distributions) (source: `scripts/pypi_release_wheels.sh`).
- Release helper defaults to `CMAKE_GENERATOR="Unix Makefiles"` when `ninja` is not available, avoiding hard dependency on `ninja` for local no-isolation release builds (source: `scripts/pypi_release_wheels.sh`).
- Release helper preflights `build`, upload-time `twine`, Linux `auditwheel`, and Linux `patchelf` before starting the release build so missing tooling fails early with actionable messages (source: `scripts/pypi_release_wheels.sh`).
- Release helper defers authentication to Twine defaults (`~/.pypirc` / keyring) unless `TWINE_USERNAME` or `TWINE_PASSWORD` are explicitly provided in env (source: `scripts/pypi_release_wheels.sh`).
- On Linux, release helper repairs `linux_x86_64` wheels via `auditwheel` to produce PyPI-accepted manylinux wheel tags before upload (source: `scripts/pypi_release_wheels.sh`).
...
- For SKBUILD wheel builds, top-level CMake package/CLI install rules are skipped (`CMakeLists.txt` early return on `SKBUILD`) so Python wheels do not accidentally include CMake package artifacts; Python install rules place `watpocket_ext` in package root and install `watpocket_lib` via file-install in the same directory with module `INSTALL_RPATH=$ORIGIN`, while native CMake `pythoninstall` keeps `watpocket_lib` in `platlib/lib` with `INSTALL_RPATH=$ORIGIN/lib` (source: `CMakeLists.txt`, `src/python/CMakeLists.txt`).
...
- Chemfiles warning handling is centralized via a one-time global callback install in `watpocket.cpp`; known noisy PDB message `PDB reader: ignoring custom space group ...` is filtered, while other chemfiles warnings are still forwarded to stderr with `[chemfiles]` prefix.
