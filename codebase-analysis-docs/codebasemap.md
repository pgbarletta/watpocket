...
- CPM remains enabled and currently fetches Catch2 + CLI11 if absent; wheel-style builds with both `BUILD_TESTING=OFF` and `WATPOCKET_BUILD_CLI=OFF` now skip the CPM download path entirely and use a no-op fallback (source: `Dependencies.cmake`, `cmake/CPM.cmake`).
...
- CPM bootstrap prefers a cached `CPM.cmake` path from `CPM_SOURCE_CACHE` and otherwise downloads the file; wheel-style builds short-circuit to an offline no-op stub before any download attempt (source: `cmake/CPM.cmake`).
