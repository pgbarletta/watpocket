...
- CPM remains enabled and currently fetches Catch2 + CLI11 if absent; wheel-style builds with both `BUILD_TESTING=OFF` and `WATPOCKET_BUILD_CLI=OFF` now fall back to a no-op CPM stub when the bootstrap download is unavailable (source: `Dependencies.cmake`, `cmake/CPM.cmake`).
- CPM remains enabled and currently fetches Catch2 only when `BUILD_TESTING=ON`; CLI11 is fetched only when `WATPOCKET_BUILD_CLI=ON` (source: `Dependencies.cmake`, `cmake/CPM.cmake`).
- CPM bootstrap prefers a cached `CPM.cmake` path from `CPM_SOURCE_CACHE` and otherwise downloads the file; if the download fails during wheel-style builds, it now falls back to an offline no-op stub (source: `cmake/CPM.cmake`).
...
- Release helper `scripts/pypi_release_wheels.sh` enforces clean-tree, exact-tag releases, builds wheel artifacts only, and uploads wheel files only (no source distributions) with CMake overrides `WATPOCKET_VERSION` + `GIT_SHA` set from git metadata; to avoid local `build/` module shadowing it executes wheel builds from a temporary directory and supports either Python `pip` or `build` tooling, both run without build isolation for reproducible offline/local-tooling releases (source: `scripts/pypi_release_wheels.sh`).
