...
- Release helper `scripts/pypi_release_wheels.sh` enforces clean-tree, exact-tag releases, builds wheel artifacts only, and uploads wheel files only (no source distributions) with CMake overrides `WATPOCKET_VERSION` + `GIT_SHA` set from git metadata; to avoid local `build/` module shadowing it executes wheel builds from a temporary directory and supports either Python `pip` or `build` tooling (source: `scripts/pypi_release_wheels.sh`).
