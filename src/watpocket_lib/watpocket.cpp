...
namespace {
  ...
  struct ChainResidKey
  {
    ...
    bool operator==(const ChainResidKey &other) const noexcept { return chain == other.chain && resid == other.resid; }
  };
  ...
  {
    std::transform(
      text.begin(), text.end(), text.begin(), [](const unsigned char c) { return static_cast<char>(std::toupper(c)); });
    return text;
  }
  ...
  std::int64_t parse_int64(const std::string &text)
  {
    ...
    try {
      ...
      if (parsed != text.size()) { throw std::runtime_error("invalid integer: '" + text + "'"); }
  ...
  Parm7FormatSpec parse_parm7_format_line(const std::string &line)
  {
    ...
    if (width == 0U) { throw std::runtime_error("invalid parm7 %FORMAT width in line: '" + line + "'"); }
    ...
    const std::string &section_name)
  {
    ...
    if (iterator == sections.end()) { throw std::runtime_error("parm7 section '" + section_name + "' is missing"); }
    ...
    const std::size_t expected_count)
  {
    ...
    if (section.format.type != 'a') {
      throw std::runtime_error(
        "parm7 section '" + section_name + "' has unexpected format type; expected character ('a')");
    }
    ...
    const std::size_t expected_min_count)
  {
    ...
    if (section.format.type != 'i') {
      throw std::runtime_error(
        "parm7 section '" + section_name + "' has unexpected format type; expected integer ('i')");
    }
  ...
  std::unordered_map<std::string, Parm7Section> parse_parm7_sections(const std::filesystem::path &parm7_path)
  {
    ...
    if (!input) { throw std::runtime_error("could not open parm7 topology file '" + parm7_path.string() + "'"); }
    ...
    while (std::getline(input, line)) {
      ...
      if (trimmed.rfind("%FORMAT", 0) == 0U) {
        if (!current_flag) { throw std::runtime_error("parm7 file has %FORMAT before %FLAG"); }
        sections[*current_flag].format = parse_parm7_format_line(trimmed);
        continue;
      }

      if (current_flag) { sections[*current_flag].lines.push_back(line); }
    }
  ...
  Parm7Topology parse_parm7_topology(const std::filesystem::path &parm7_path)
  {
    ...
    if (pointers.size() < 12U) { throw std::runtime_error("parm7 POINTERS section has too few values"); }

    const auto natom = static_cast<std::size_t>(pointers.at(0));
    const auto nres = static_cast<std::size_t>(pointers.at(11));
    if (natom == 0U) { throw std::runtime_error("parm7 POINTERS indicates NATOM is zero"); }
    if (nres == 0U) { throw std::runtime_error("parm7 POINTERS indicates NRES is zero"); }
    ...
    for (const auto v : residue_pointers_i) {
      if (v <= 0) { throw std::runtime_error("parm7 RESIDUE_POINTER contains non-positive value"); }
  ...
  ResidueSelector parse_selector_token(const std::string &token)
  {
    ...
    if (raw.empty()) { throw std::runtime_error("empty residue selector in --resnums"); }

    const auto first_colon = raw.find(':');
    if (first_colon == std::string::npos) { return ResidueSelector{ std::nullopt, parse_int64(raw), raw }; }
  ...
  {
    if (selector.chain) { return *selector.chain + ":" + std::to_string(selector.resid); }
    return std::to_string(selector.resid);
  }

  std::string residue_chain_id(const chemfiles::Residue &residue)
  {
    if (const auto chainid = residue.get<chemfiles::Property::STRING>("chainid")) { return *chainid; }

    if (const auto chainname = residue.get<chemfiles::Property::STRING>("chainname")) { return *chainname; }
  ...
  std::size_t find_ca_atom_index(const chemfiles::Frame &frame, const chemfiles::Residue &residue)
  {
    ...
    for (const auto atom_index : residue) {
      if (frame[atom_index].name() == "CA") {
        if (ca_atom_index) { throw std::runtime_error("residue has multiple 'CA' atoms"); }
        ca_atom_index = atom_index;
      }
    }

    if (!ca_atom_index) { throw std::runtime_error("residue does not contain atom named 'CA'"); }
    ...
    const std::vector<ResidueSelector> &selectors)
  {
    ...
    for (const auto &selector : selectors) {
      ...
      if (selector.chain) {
        ...
        if (iterator != lookup.by_chain_and_id.end()) { matches = iterator->second; }
      } else {
        const auto iterator = lookup.by_id.find(selector.resid);
        if (iterator != lookup.by_id.end()) { matches = iterator->second; }
      }
    ...
    const std::vector<ResidueSelector> &selectors)
  {
    ...
    for (const auto &selector : selectors) {
      if (selector.chain && !topology.has_residue_chain_ids) {
        throw std::runtime_error(
          "selector '" + selector_display(selector)
          + "': parm7 topology has no RESIDUE_CHAINID section; chain-qualified selectors are not supported");
      }

      if (selector.resid <= 0) {
        throw std::runtime_error(
          "selector '" + selector_display(selector) + "': residue id must be positive for parm7 topology");
      }
    ...
    for (std::size_t i = 0; i < atom_indices.size(); ++i) {
      ...
      if (atom_index >= positions.size()) {
        throw std::runtime_error(label + " atom index " + std::to_string(atom_index)
                                 + " is out of bounds for frame with " + std::to_string(positions.size()) + " atoms");
      }
  ...
  std::vector<WaterOxygenRef> collect_water_oxygen_refs(const chemfiles::Frame &frame)
  {
    ...
    for (const auto &residue : residues) {
      ...
      if (water_resnames.count(uppercase_copy(residue.name())) == 0U) { continue; }

      for (const auto atom_index : residue) {
        if (water_oxygen_names.count(uppercase_copy(frame[atom_index].name())) == 0U) { continue; }
  ...
  std::vector<WaterOxygenRef> collect_water_oxygen_refs(const Parm7Topology &topology)
  {
    ...
    for (std::size_t residue_index = 0; residue_index < residue_count; ++residue_index) {
      if (water_resnames.count(uppercase_copy(topology.residue_labels[residue_index])) == 0U) { continue; }
      ...
      for (std::size_t atom_index = begin; atom_index < end; ++atom_index) {
        if (water_oxygen_names.count(uppercase_copy(topology.atom_names[atom_index])) == 0U) { continue; }
  ...
  std::string normalize_pdb_field(const std::string &text, const std::size_t width)
  {
    ...
    if (normalized.size() > width) { normalized.resize(width); }
    return normalized;
  }
  ...
  void write_pdb_atom_record(std::ostream &out, const std::size_t serial, const PdbAtomRecord &atom)
  {
    ...
    const auto element =
      normalize_pdb_field(atom.element.empty() ? guess_element_symbol(atom.atom_name) : atom.element, 2U);
    ...
    const std::vector<std::int64_t> &water_residue_ids)
  {
    ...
    if (water_residue_ids.empty()) { return water_atoms; }
    ...
    for (const auto &residue : residues) {
      ...
      if (!std::binary_search(water_residue_ids.begin(), water_residue_ids.end(), residue_id)) { continue; }

      const auto residue_name = residue.name();
      if (water_resnames.count(uppercase_copy(residue_name)) == 0U) { continue; }

      const auto chain_id = residue_chain_id(residue);
      for (const auto atom_index : residue) {
        if (atom_index >= positions.size()) { continue; }

        const auto &pos = positions.at(atom_index);
        water_atoms.push_back(PdbAtomRecord{
          frame[atom_index].name(), residue_name, chain_id, residue_id, { pos[0], pos[1], pos[2] }, "" });
      }
    }
    ...
    const std::vector<std::int64_t> &water_residue_ids)
  {
    ...
    for (const auto resid : water_residue_ids) {
      ...
      if (residue_index >= topology.residue_labels.size()) { continue; }
      ...
      for (std::size_t atom_index = begin; atom_index < end; ++atom_index) {
        ...
        const auto &pos = positions.at(atom_index);
        water_atoms.push_back(PdbAtomRecord{
          topology.atom_names[atom_index], residue_name, chain_id, resid, { pos[0], pos[1], pos[2] }, "" });
      }
    }
  ...
  void
    write_csv_row(std::ostream &out, const std::size_t frame_number, const std::vector<std::int64_t> &water_residue_ids)
  {
    out << frame_number << ",\"";
  ...
  double median_of(std::vector<std::size_t> values)
  {
    ...
    if ((values.size() % 2U) == 1U) { return static_cast<double>(values[mid]); }
    ...
    const std::unordered_map<std::int64_t, std::size_t> &water_presence_counts)
  {
    ...
    if (!waters_per_frame.empty()) {
      ...
      summary.mean_waters =
        static_cast<double>(std::accumulate(waters_per_frame.begin(), waters_per_frame.end(), std::size_t{ 0 }))
    ...
    for (const auto &[resid, count] : water_presence_counts) {
      presences.push_back(TrajectoryWaterPresence{ resid,
        count,
        waters_per_frame.empty() ? 0.0 : static_cast<double>(count) / static_cast<double>(waters_per_frame.size()) });
    }

    std::sort(presences.begin(), presences.end(), [](const auto &a, const auto &b) {
      if (a.frames_present != b.frames_present) { return a.frames_present > b.frames_present; }
  ...
  void
    write_hull_pdb_records(std::ostream &out, const HullGeometry &hull, const std::vector<PdbAtomRecord> &water_atoms)
  {
    std::size_t serial = 1;

    for (const auto &vertex : hull.vertices) {
      write_pdb_atom_record(out, serial++, PdbAtomRecord{ "C", "ANA", "", 1, vertex, "C" });
    }
    ...
    for (const auto &atom : water_atoms) { write_pdb_atom_record(out, serial++, atom); }
  }
...
std::vector<ResidueSelector> parse_residue_selectors(std::string_view csv)
{
  try {
    ...
    while (std::getline(stream, token, ',')) { selectors.push_back(parse_selector_token(token)); }

    if (selectors.empty()) { throw std::runtime_error("--resnums must contain at least one residue selector"); }
  ...
  std::string_view label)
{
  try {
    ...
    if (frame_number == 0U) { throw Error("frame number must be >= 1"); }
    ...
    if (frame_count == 0U) { throw Error("trajectory contains no frames: '" + trajectory_path.string() + "'"); }
    ...
    if (frame.size() != topology_atom_count) {
      throw Error("atom-count mismatch between topology and trajectory: topology has "
                  + std::to_string(topology_atom_count) + " atoms, trajectory frame " + std::to_string(frame_number)
                  + " has " + std::to_string(frame.size()) + " atoms");
    }
  ...
  const std::vector<std::int64_t> &water_residue_ids)
{
  ...
  if (!out) { throw Error("could not open draw output file '" + output_path.string() + "'"); }
  ...
  out << "cmd.select(\"watpocket\", \"watpocket_input and solvent and " << make_pymol_resi_selector(water_residue_ids)
      << "\")\n";
